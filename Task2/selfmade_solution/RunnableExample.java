...
public class RunnableExample implements Runnable {
    ...
    private Thread t;
    public final String csvFile;
    public final String tag;

    RunnableExample(String name, String tag){
        this.csvFile = name;
        this.tag = tag;
    }
    public void start() {
        if(t==null){
            t= new Thread(this,csvFile);
            t.start();
        }
    }
...
/*		//create securityGroup
        ...
        System.out.println("Command: sudo apt -y install openjdk-8-jdk\n");
        SshCommands.sendCommandViaSsh(instancePublicIP, keyPairPath, "sudo apt -y install openjdk-8-jdk");

        System.out.println("Command: sudo java -version\n");
        SshCommands.sendCommandViaSsh(instancePublicIP, keyPairPath, "sudo java -version");
        ...
        try {
            SshCommands.whenDownloadFileUsingJsch_thenSuccess(instancePublicIP, keyPairPath, tag, "output.csv");
        } catch (JSchException e) {
            e.printStackTrace();
        } catch (SftpException e) {
            e.printStackTrace();
